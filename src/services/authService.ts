...
export class AuthService {
  ...
  async signUp(data: SignUpData): Promise<AuthResult> {
    try {
      ...
      // Create user account with Supabase Auth with email confirmation DISABLED
      ...
      if (authError) {
        ...
        if (authError.message.includes('email_address_invalid')) {
          return { success: false, message: 'Please enter a valid email address. Make sure it follows the format: example@domain.com' };
  ...
  async verifyOTP(email: string, otp: string): Promise<AuthResult> {
    try {
      ...
      // Also update the auth user's email_confirmed_at to mark as confirmed
